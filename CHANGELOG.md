...
### 🚧 In progress (handoff — adoption pass continuation)
1. **Tool-call text parser module** (not started): create `local_coding_agent/tool_call_parser.py` + tests — API contract: `extract_tool_calls(text, allowed_names=None) -> ToolCallExtraction(calls, remaining_text, formats_detected)`; formats: qwen `<tool_call>` JSON, qwen XML `<function=NAME><parameter=K>`, llama `<|python_tag|>{json}`, mistral `[TOOL_CALLS] [...]`; promote only allowlisted names, truncated tails left in text, never raise.
2. **Parser → controller integration**: in `controller/_controller.py` run-loop, when `_decode_content_tool_call` returns None, fall back to `extract_tool_calls(content, allowed_names={d["function"]["name"] for d in self._tools_for_task(task)})`; promote to message tool_calls + audit event `text_tool_call_promoted`.
3. **Compaction → CLI REPL wiring**: in `cli/_handlers2.py` multi-turn loop, guard each model call with `fit_history(messages, profile.num_ctx - 1024, sticky_boundary)`; persist boundary per session state.
4. **VRAM auto-fit → desktop model load wiring**: in `desktop/server/_handlers.py` `_handle_model_load`, accept `num_ctx:"auto"` on the GGUF branch: `read_gguf_ctx_params` + free VRAM from `get_nvidia_gpu_telemetry()` → `max_fitting_ctx` → set `llama_num_ctx` before launch; explicit int behavior must stay byte-identical.
5. Full suite currently green (`pytest tests/`); keep it green per change.
